                ...
                <div className="max-w-6xl mx-auto">
                        ...
                        <div className="lg:col-span-2">
                            <div className="mb-6">
                                ...
                                <p className="text-gray-400 max-w-md">
                                    Building scalable, high-performance web applications with clean code and modern design.
                            ...
                            <div className="flex flex-wrap gap-4">
                                {quickContacts.map((contact, index) => (
                                    <a
                                        key={index}
                                        href={contact.href}
                                        className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-gray-300 transition-all duration-300 ${contact.contentCss}`}
                                    >
                                        {contact.icon}
                                        <span className={`text-sm ${contact.labelCss}`}>{contact.label}</span>
                                    </a>
                                ))}
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <ul className="space-y-3">
                                {services.map((service, index) => (
                                    <li key={index}><a href={service.href} className="text-gray-400 hover:text-purple-300 transition-colors">{service.name}</a></li>
                                ))}
                            </ul>
                        </div>
                        ...
                        <div>
                            ...
                            <ul className="space-y-3">
                                {quickLinks.map((link, index) => (
                                    <li key={index}><a href="#about" className="text-gray-400 hover:text-purple-300 transition-colors">{link.name}</a></li>
                                ))}
                            </ul>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between pt-8 border-t border-white/10">
                        <div className="flex items-center text-gray-400 text-sm mb-4 md:mb-0">
                            <span>© {currentYear} KushalSojitra. All rights reserved.</span>
                        </div>
